...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Join(emps, emp => emp.Mgr, mgr => mgr.EmpNo, (emp, mgr) => new { Employee = emp.EName, Manager =mgr.EName}).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d })
            .SelectMany(ed => grades.Where(g => ed.e.Sal >= g.Losal && ed.e.Sal <= g.Hisal)
